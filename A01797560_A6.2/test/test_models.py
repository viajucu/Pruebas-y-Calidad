...
class TestModels(unittest.TestCase):
    ...
    def test_hotel_total_rooms_debe_ser_positivo(self):
        with self.assertRaises(ValueError):
            Hotel(hotel_id="H1", name="X", city="CDMX", total_rooms=0)

    def test_customer_email_invalido(self):
        with self.assertRaises(ValueError):
            Customer(customer_id="C1", full_name="Ana", email="correo_malo")

    def test_reservation_fechas_invalidas(self):
        with self.assertRaises(ValueError):
            Reservation(
                reservation_id="R1",
                hotel_id="H1",
    ...
    def test_serializacion_roundtrip(self):
        ...
        self.assertEqual(Reservation.from_dict(r.to_dict()).reservation_id, "R1")
